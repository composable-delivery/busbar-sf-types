...
impl ModularGenerator {
    ...
    fn generate_trait_impls(
        ...
        let impl_code =
            generate_all_trait_impls(struct_names, &fields_by_type, &self.config.trait_config);

        let path = output_dir.join("trait_impls.rs");
        let content = format!(
            "{}//! Auto-generated trait implementations for Salesforce types.\n//!\n//! These implementations require the `full` feature to be enabled.\n\n// Import all types from the monolithic generated file\nuse crate::generated_salesforce_types::*;\n\n{}\n",
            self.file_header(),
            impl_code
        );
        fs::write(&path, &content)?;
...
"#,
        ...
        // Trait implementations (only available with full feature for now)
        // This is because trait impls reference types across modules
        content.push_str("// Trait implementations (requires full feature)\n");
        content.push_str("#[cfg(feature = \"full\")]\n");
        content.push_str("mod trait_impls;\n\n");

        // Backward compatibility: full generated file
        content.push_str("// Backward compatibility: monolithic generated file\n");
        content.push_str("#[cfg(feature = \"full\")]\n");
        content.push_str("#[doc(hidden)]\n");
        content.push_str("pub mod generated_salesforce_types;\n");
